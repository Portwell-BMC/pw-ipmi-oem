...
static constexpr const int firstAgentIndex = 0;

static constexpr const uint8_t maxDirEntries = 1; // Maximum directory entries
...
static constexpr const uint32_t mdr2SMBaseAddress =
    0x98000000; // Base address of VGA share memory
...
static constexpr const uint32_t pageMask =
    0xfffff000; // To make data become n times of page
...
static constexpr const size_t smbiosSMMemoryOffset =
    0x80000; // Offset of VGA share memory
static constexpr const size_t smbiosSMMemorySize =
    512 * 1024; // Total size of VGA share memory
...
*/
  ...
  private:
    ...
    bool openP2A(uint32_t addr, uint32_t areaSize);
    bool closeP2A();
};
