...
bool SharedMemoryArea::openP2A(uint32_t addr, uint32_t areaSize)
{
...
/*
    if (ioctl(p2aCtrlFd, ASPEED_P2A_CTRL_IOCTL_GET_MEMORY_CONFIG, &map))
    {
        close(p2aCtrlFd);
        p2aCtrlFd = -1;
        phosphor::logging::log<phosphor::logging::level::ERR>(
...
bool SharedMemoryArea::closeP2A()
{
